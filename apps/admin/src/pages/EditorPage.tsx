...
export function EditorPage() {
  ...
  const preview = () => {
    ...
    w?.document.write(`<title>${item.title}</title><style>body{font:18px/1.7 system-ui;max-width:760px;margin:60px auto;padding:20px;color:#24332d}img{max-width:100%}.meta{color:#6f7b73;font-size:14px;text-transform:uppercase;letter-spacing:.08em}</style><p class="meta">${item.category} · ${item.authorName}</p><h1>${item.title}</h1>${html}`)
    w?.document.close()
  }
  ...
  return <>
    ...
    <div className="editor-layout">
      <section className="editor-main">
        <input className="title-input" aria-label="Title" placeholder="Untitled masterpiece" value={item.title} onChange={e => {
          setItem({ ...item, title: e.target.value, slug: item.slug || slug(e.target.value) })
          setSaved(false)
        }} />
        <input className="slug-input" aria-label="Slug" value={item.slug} onChange={e => {
      ...
      <aside className="inspector">
        ...
        <hr />
        <h3>Details</h3>
        <label>Excerpt<textarea rows={4} value={item.excerpt} onChange={e => patch({ excerpt: e.target.value })} /></label>
        ...
        <hr />
        <h3>SEO</h3>
        <label>SEO title<input value={item.seoTitle} onChange={e => patch({ seoTitle: e.target.value })} /></label>
        <label>Description<textarea rows={3} value={item.seoDescription} onChange={e => patch({ seoDescription: e.target.value })} /></label>
