...
async function repairChinesePrimary(input: { sourceName: string; sourceUrl: string; title: string; body: string }, credit: string) {
  const prompt = `请把以下新闻素材改写成完整、自然的简体中文新闻稿。标题、摘要、SEO 标题、SEO 描述和正文必须以简体中文为主；不得保留英文标题作为中文标题，也不得在标题前后加上来源名称、[来源]、【来源】或网站品牌。标题必须准确概括新闻事件，而不是复制来源标题。只能依据素材，不得添加事实。务必保留素材中的具体数字、日期、金额、人名与地名，不要将其模糊化。正文至少五段，并以这句作为最后一行来源标注：${credit}。imageSearchQuery 必须保留简短英文，供图库检索。仅返回符合 JSON schema 的完整对象，不要解释。\n\n来源标题：${input.title}\n来源正文：\n${input.body.slice(0, 16_000)}`
