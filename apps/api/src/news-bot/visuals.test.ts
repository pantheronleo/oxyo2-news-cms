...
const { resolveVisual } = await import('./visuals.js')
...
describe('news bot visual resolver', () => {
  it('uses a Pexels asset before trying publisher or AI fallbacks', async () => {
    ...
    const result = await resolveVisual({ query: 'market', prompt: 'A market', purpose: 'cover', allowSourceReference: false, articleUrl: 'https://source.example/story' })
  ...
  it('tries broader Pexels thumbnail queries before using another visual source', async () => {
    ...
    fetchMock.mockResolvedValueOnce(new Response(png, { headers: { 'content-type': 'image/png' } }))
    const result = await resolveVisual({ query: 'too specific', fallbackQueries: ['broader market'], prompt: 'A market', purpose: 'cover', allowSourceReference: false, articleUrl: 'https://source.example/story' })
