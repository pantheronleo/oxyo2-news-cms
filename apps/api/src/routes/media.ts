...
export async function mediaRoutes(app: FastifyInstance) {
  ...
  app.delete('/:id', async (req,reply) => { const id=(req.params as any).id; const item=await prisma.media.findUnique({where:{id},include:{_count:{select:{covers:true}}}}); if(!item)return reply.code(404).send({error:{code:'NOT_FOUND',message:'Media not found'}}); if(item._count.covers)return reply.code(409).send({error:{code:'MEDIA_IN_USE',message:'Remove this media from content before deleting it'}}); await prisma.media.delete({where:{id}}); await unlink(item.path).catch(err=>app.log.warn(err)); return reply.code(204).send() })
}
